...
const { MonthStartDateRule } = require('virtwallet-lib/month-start-date-rule');
...
exports.parseCsvFile = async (stream, monthStartDateRule) => {
  const transactions = await readTransactions(stream, monthStartDateRule);

  logger.info(`Total transactions parsed [${transactions.length}]`);
...
async function readTransactions(fileStream, monthStartDateRule) {
  ...
  logger.debug("Finished parsing transactions", { results });
  return results;
}
...
function transformPosTx(transaction) {
  ...
  logger.debug(`Card used in POS: [${card}] for transaction ${transaction.id}`);
